...
func generate_map():
	...
	init_map_bounds()
	clear_tiles()
	var is_maze:bool = rng.randi()%100 < 50
	...
	var extra_rooms: int = rng.randi_range(1, 5)
	for i in range(extra_rooms):
		t_room = generate_room()
		var to_fit: Array = map_grid.get_cells_to_fit_room(t_room, 2, -1) # margin 2 to avoid having the room in a border of the map
		if to_fit.size() == 0:
			print("unable to fit room")
			break
	...
	if is_maze:
		map_grid.ground.set_grid_array(PCG.generate_maze_path(map_grid.ground.get_grid_array(), $Ground.tile_set.find_tile_by_name('ground_green'), 0.3, door_entrance_pos,  map_grid.ground.get_grid_array(), rng, Vector2(1, 1)))
	else:
		map_grid.ground.set_grid_array(PCG.generate_path_with_walkers(map_grid.ground.get_grid_array(), 2000, $Ground.tile_set.find_tile_by_name('ground_green'), 0.5, rng, door_entrance_pos, map_grid.ground.get_grid_array()))
	...
	print(available_items_cells.size())
